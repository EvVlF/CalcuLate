...
class Validation {
    ...
    private static ArrayDeque<StringBuilder> validateEnteredExpression() throws NotCorrectExpression {
        if (!((Character.isDigit(enteredExpression.charAt(0))) && (Character.isDigit(enteredExpression.charAt(enteredExpression.length() - 1))))) {
        ...
        for (int i = 0; i < enteredExpression.length(); i++) {
            if (' ' == enteredExpression.charAt(i)) {
                ...
                while ((Character.isDigit(enteredExpression.charAt(i)) || '.' == enteredExpression.charAt(i))) {
                    if (('.' == enteredExpression.charAt(i)) && (enteredExpression.charAt(i - 1) == enteredExpression.charAt(i))) {

        ...
        while (!validatedExpression.isEmpty()) {
            ...
            if (Character.isDigit(charAt0)) {
                ...
                switch (charAt0) {
                    case '+':
                    case '-':
                        while (!asStack.isEmpty() && ('+' == asStack.peek().charAt(0) || '+' == Objects.requireNonNull(asStack.peek()).charAt(0) || '*' == Objects.requireNonNull(asStack.peek()).charAt(0) || '/' == (Objects.requireNonNull(asStack.peek())).charAt(0))) {
                    ...
                    case '*':
                    case '/':
                        while (!asStack.isEmpty() && ('*' == asStack.peek().charAt(0) || '/' == (Objects.requireNonNull(asStack.peek())).charAt(0))) {
                            preReversedPolishNotation.add(asStack.pop());
                        }
                        asStack.push(elementOfValidatedExpression);
                        break;
